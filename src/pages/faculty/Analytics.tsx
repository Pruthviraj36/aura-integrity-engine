  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-2">
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-foreground uppercase aura-text-glow">Institutional Intelligence</h1>
        ...
        <div className="glass-card aura-glow border-none px-6 py-3 rounded-2xl flex flex-col items-end">
          <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Performance Index</p>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <Card className="glass-card aura-glow border-none overflow-hidden group">
          <CardHeader className="bg-primary/5 border-b border-border/10 px-6 py-4">
            <CardTitle className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] italic">Top Performance Distribution</CardTitle>
          </CardHeader>
            ...
            {topStudents.length > 0 ? (
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={topStudents}>
                  ...
                  />
                  <Bar dataKey="attainment" fill="hsl(var(--primary))" radius={[8, 8, 2, 2]} barSize={35} className="filter drop-shadow-[0_0_8px_rgba(var(--primary),0.4)]" />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <Card className="glass-card aura-glow border-none overflow-hidden group">
          <CardHeader className="bg-primary/5 border-b border-border/10 px-6 py-4">
            <CardTitle className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] italic">Engagement Vector Map</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <ResponsiveContainer width="100%" height={280}>
              <RadarChart data={radarData}>
                <PolarGrid stroke="rgba(255,255,255,0.05)" />
                <PolarAngleAxis dataKey="subject" tick={{ ...tickStyle, fill: 'hsl(var(--foreground))', opacity: 0.6 }} />
                <PolarRadiusAxis tick={{ ...tickStyle, fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} domain={[0, 100]} />
        ...
        <Card className="glass-card aura-glow border-none lg:col-span-2 overflow-hidden">
          <CardHeader className="bg-primary/5 border-b border-border/10 px-8 py-5 flex flex-row items-center justify-between">
            <CardTitle className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] italic">Attendance Telemetry Trend</CardTitle>
